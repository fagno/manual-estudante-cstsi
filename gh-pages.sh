...
if [[ $1 == "--no-history" || $1 == "-nh" ]]; then
    ...
    git checkout --orphan gh-pages main 1>/dev/null || (echo "It was not possible to change to gh-pages branch" && exit -1)
else
    echo "Change history in the gh-pages branch will be kept."
    git checkout main -B gh-pages 1>/dev/null || (echo "It was not possible to change to gh-pages branch" && exit -1)
fi
echo ""
